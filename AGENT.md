...
├── src/                          # Python (stdlib only). The server treats the folder ABOVE src/ as the project root (web root, NSE_DATA/, reference-data/, scanner-presets/, Download-NSE-Bhavcopy.ps1); `--dir` overrides
...
│   └── tv_report.py              # Markdown + CSV report of a full "verify all stocks against TradingView" run (pure functions, no I/O) — see "Full Price Verification Against TradingView"
...
│   ├── CorporateActions.csv      # Split/bonus/demerger corporate actions (ISIN, SYMBOL, EXDATE, SUBJECT). A permanent ARCHIVE: NSE serves only a rolling window, so the downloader replaces the rows inside that window with NSE's fresh copy and KEEPS rows that have aged out of it (see "Corporate-actions archive"). Written by Download-NSE-Bhavcopy.ps1 — see "Stock Split/Bonus Price Adjustment"
│   └── DemergerAdjustments.csv   # TradingView-derived price corrections (ISIN, SYMBOL, EXDATE, FACTOR, STATUS, CHECKED_AT, NOTE), written by the Data Quality "Adjust prices from TradingView" button, applied on every processing run — see "TradingView-Derived Demerger Corrections". Delete a row to undo that correction
├── .claude/
...
- `GET /api/presets` — Load screener presets from `scanner-presets/presets.json`.
- `POST /api/presets` — Save screener presets to `scanner-presets/presets.json`.
...
**All eight report sections default to collapsed** (`.dq-section-header`/`.dq-section-body`, `dqToggleSection(id)` in `js/data-quality.js` — same click-header-to-toggle-a-body-div convention as the Stocks tab's `toggleScannerFilters()`), so the tab opens compact instead of dumping every table at once; the seven summary stat cards above them (including "Unadjusted Corp Actions" and "Price Adjustments Applied", the latter showing a red/orange "⚠ N flagged" once verified) are always visible regardless of which sections are expanded.
...
1. **PowerShell** (`Download-NSE-Bhavcopy.ps1` and `Start-Dashboard.ps1`, both updated identically): requests a **3-year window** from the endpoint on every run (a few hundred split/bonus rows — cheap enough that incremental-fetch state-tracking isn't worth the complexity; `-CorpActionsFrom` widens it once for a backfill), filters to rows whose `subject` matches `bonus|split|sub-division|consolidation of equity shares` (a loose pre-filter — real parsing happens server-side), and MERGES them into `reference-data/CorporateActions.csv` (`ISIN,SYMBOL,EXDATE,SUBJECT`; git-tracked; it moved out of `NSE_DATA/` — see "Project layout"). It is an archive, not a mirror: see "Corporate-actions archive" below.
2. **`nse_server.py`**: `needs_processing()` now also watches `reference-data/CorporateActions.csv`'s mtime. `process_data()` runs `load_split_bonus_events()` → `bridge_split_induced_isin_changes()` → `apply_split_adjustments()` on `dailyBySymbol` immediately after grouping/sorting, before any indicator (`sma20`, 52W high/low, `adr`, `changePct`, `monthlyChangePct`, and the equal-weight index) is computed from it — so every downstream consumer sees already-continuous, already-adjusted prices with no separate adjustment step of its own.
...
- Pre-2022 combined/abbreviated subject phrasings aren't parsed (see above) — irrelevant while the archive holds only recent events, but a `-CorpActionsFrom` backfill of older history would need broader regexes to price-adjust them (harmless meanwhile: they are ignored, see "Corporate-actions archive").
...
## Completed: Full Price Verification Against TradingView (all stocks + report)

**Purpose (from the user):** "run price data verification for all stocks that we have against TradingView data, using TradingView debug mode, same as we have now — I will run it manually occasionally and make sure we do not deviate too much. After the data match is done, create a report which we can use to analyse and check if we need to make any improvements."

**Flow:** Data Quality → the eighth collapsible section "Full Price Verification (all stocks vs TradingView)" → "Verify all stocks" → `POST /api/tv-full/start[?limit=N][&resume=1]` → `run_tv_verify_full()` (background thread; read-only — no price changes, no reprocess). The universe is every stock in `dailyBySymbol` (2,899 today: EQ+BE stocks that traded on the latest day), **most-traded first** (`collect_verify_universe()`, so a stopped run has covered the stocks that matter most). Each stock goes through `_verify_stock()` — the same TradingView read + `tv_adjust.compare_series()` grading as the adjusted-stocks "Verify against TradingView" (which now calls the same helper; see "Verifying Adjusted Prices Against TradingView") — over its whole stored window (~510 days). **Speed:** ~0.6–1 s per stock (top-100 by turnover took 94 s including one 25 s timeout), i.e. roughly 35–45 minutes for all stocks; the tab shows a progress bar, live counts and an ETA. The Scope select offers all / top 500 / top 100 by turnover (`limit`, a quick sample). While it runs the user's TradingView chart flips through every symbol (restored afterwards), so TradingView must be left alone, visible, and the PC awake.

**Stop / resume / crash safety:** progress is checkpointed every 100 stocks to `NSE_DATA/TradingViewFullVerification.json` (atomic tmp + `os.replace`). *Stop* (`POST /api/tv-full/stop`, `_tv_full_stop` event) finishes the current stock, saves and writes a partial report; a lost TradingView connection ends the run the same way ("Stopped early"); any other exception flushes what was verified in `finally`. *Resume* (`?resume=1`, the button shows "Resume (N left)") continues with only the stocks not yet done — but **only against the same processed-data snapshot** (`meta.dataStamp` = `processed_data.json` mtime in ms; after a refresh/reprocess prices, and so results, may differ): otherwise the request is ignored with a warning and a fresh run starts. A resumed run uses the saved run's scope. Starting a fresh run over an interrupted one discards the partial.

**What is stored per stock** (`stocks[isin]` in the file): everything `compare_series` returns (`status` match/minor/major/inconclusive, `bars`, `barsOff`, `maxDevPct`, `worstDate`, `firstOff`/`lastOff`, `causes`, `note`) plus `reason` (for inconclusive: `not-found` / `tv-error` / `few-bars` / `predates-history`), `endDevPct` (deviation on the last shared bar — is *today's* price right?), `causeDetails` (structured: `kind` = `tv-extra-adjustment` / `adjustments-differ` / `tv-no-adjustment`, `date`, `ourFactor`, `tvFactor`), and the stock's `symbol`, `series`, `turnover` (Cr; the bhavcopy value is in lakhs), `close`, `events` (its adjustments as `[[exDate, factor]]`), `tvOnlyN`/`oursOnlyN`. Per-date calendar differences are kept as counts (`tvOnlyByDate`/`oursOnlyByDate`) and reduced to `calendar` (dates missing on one side for at least half the compared stocks) — same rule as the adjusted-stocks verify.

**The report** (`src/tv_report.py`, pure functions; `build_report(payload, prev, corp_index)` → Markdown + CSV) is written when a run ends (complete or stopped) to `NSE_DATA/verification-reports/TradingView-Verification_<date>_<HHMM>[_partial].md` and `.csv`, and offered as two download links in the tab (`GET /api/tv-full/report?format=md|csv`; only the file named in the run's own meta is ever served, reduced to a basename). The Markdown has: run info + method; a summary table (match / minor / major / could not be compared, by reason) with key numbers (bars compared and off, median/95th-percentile/worst per-stock deviation, stocks whose *latest* bar differs by >1%, how the dashboard's own price-adjusted stocks fared vs stocks it never adjusted); **changes since the previous run** (newly flagged / resolved / worse / better — the previous *complete* run is kept as `TradingViewFullVerification.prev.json`); **major differences grouped by cause** (TradingView adjusts for an action we have no event for → our adjustment differs → TradingView shows no adjustment for our event → isolated bars only), most-traded first; minor differences; the stocks that could not be compared; a by-series (EQ/BE) table; calendar differences; and a "reading this report" guide mapping each cause to a likely improvement. For "missing event" differences it cross-references `reference-data/CorporateActions.csv` (`load_corp_action_index()`, `corp_action_hints()`): a feed row within 5 days of TradingView's level shift means the action IS in the feed but not price-adjusted (a demerger, or one older than the "Not Price-Adjusted" list's ~370-day window), no row means the feed doesn't carry it at all (rights issues are filtered out by the downloader). The CSV has one row per verified stock (UTF-8 BOM for Excel) with all of the above incl. `cause_group` and `nse_corporate_action`. The tab shows the summary, the two links and a table of the flagged stocks (`GET /api/tv-full/results`, major first then most-traded; "Show: major only" filter).

**Ties in with the adjusted-stocks list:** when a run ends, the results for price-adjusted stocks whose recorded adjustments still equal their current ones are merged into `TradingViewVerification.json` (`merge_full_run_into_queue()`), so stocks that matched leave the "Price Adjustments Applied" to-do queue exactly as if that list's own button had checked them (the tab reloads that list after a run).

**Concurrency:** shares `_tv_lock` with the correction and adjusted-stocks runs — each start handler answers 409 while another of the three is running (one TradingView chart), and a full run also refuses while a reprocess is in flight. Same `X-Requested-With: nse-dashboard` same-origin rule (else 403) on start and stop. `GET /api/tv-full/status` = live state (`_tv_full_state`: `status`, `step`, `current`/`total`, `counts`, `stopping`, `stopped`, `warning`, `error`) plus, when idle, `last` (`summarize_full_verification()`: meta, category counts, `remaining`, `resumable`); it never reads the file while a run is in progress.

**Two bugs found while building it (both fixed, both also affected the older TradingView features):**
- *History-loading stall:* `_JS_LOAD_BARS` kept asking TradingView for older bars while the first loaded bar's timestamp (09:15 IST) was later than midnight of the needed day — i.e. even when it was ON the needed day — and waited its 10 s "more data" timeout for each such stock (e.g. `PICCADIL`, 10.9 s instead of 0.5 s). The comparison now has a day of slack.
- *`&` in tickers:* `tv_symbol()` used to turn both `&` and `-` into `_`. TradingView keeps `&` (`NSE:M&M`; `NSE:M_M` is only an alias it re-canonicalises to `NSE:M&M`, which the loader reads as "the wrong symbol opened" and times out after 25 s), and only writes `-` as `_` (`NSE:BAJAJ_AUTO`). Eight stocks were affected (`ARE&M`, `GMRP&UI`, `GVT&D`, `J&KBANK`, `M&M`, `M&MFIN`, `S&SPOWER`, `SURANAT&P`) — none of them could ever have been verified or TradingView-corrected. Also new: `TradingViewSymbolNotFound` (a symbol TradingView doesn't have, distinct from other per-stock errors) so the report can list "not on TradingView" separately.

**Verified:** (1) a fake-TradingView harness with a synthetic 250-stock dataset — grading of match / minor / major / not-found / tv-error / too-few-bars, cause kinds and TradingView factors, most-traded-first order, the calendar finding, live counts, report + CSV contents, queue merge, the "changes since previous run" section, Stop → partial report → Resume (only the remaining stocks are re-checked, the partial report is replaced, results equal a straight run), a dropped connection, resume refused after a reprocess, `limit`, TradingView-down / no-data error paths (previous run's file untouched), an unexpected crash (checkpoint + flush kept), and the HTTP layer (403 without the header, 409 exclusion both ways, alreadyRunning, report download and path safety). (2) The real TradingView on an isolated copy of the data (`--dir` pointing at a scratch project), through the real buttons: a top-100 run (94 s: 92 match, 2 major, 5 too little history, 1 TradingView error — `M&M`, which is what exposed the `&` bug above), then a second top-100 run that was **stopped at 18/100 and resumed** ("Resume (82 left)") to completion, with the partial report replaced by the final one and the "changes since the previous run" section filled in. (3) The same per-stock code path over a random 300-stock sample that includes all 16 stocks with special characters in their ticker (237 s = 0.79 s/stock; p50 0.72 s, p90 1.06 s, p99 1.65 s): 289 match, 8 major, 3 too little history, no not-found/TradingView errors. All 8 are "TradingView also scales prices before <date>" (`M&MFIN`, `DELPHIFX`, `ONELIFECAP`, `SEPC`, `GLOBE`, `CCAVENUE`, `JAYKAY`, `ATL`) with a **latest-bar deviation of 0 for every one — today's prices are right, only history before that date differs** — so roughly 2–3% of stocks carry a TradingView-only adjustment (looks like rights issues) that the dashboard doesn't. The two slow stocks (`UMIYA-MRO` 10.8 s, `ABMKNO` 3.0 s) are ones whose TradingView history starts later than ours, so the loader asks for more data that doesn't exist and waits out its 10 s timeout (about 1 stock in 300).

**Findings on real data:** `RAYMOND` (×0.3333 before 14-May-2025) and `LLOYDSENGG` (×0.8843 before 28-Apr-2025) are major differences among the top 100 by turnover, both "TradingView adjusts for an action the dashboard has no event for". `RAYMOND`'s 14-May-2025 **Demerger IS in `CorporateActions.csv`** but is neither price-adjusted nor listed under "Not Price-Adjusted" (its ~370-day window doesn't reach back that far while the dashboard keeps ~510 days of prices) — the blind spot already noted above, now confirmed by a real stock; `LLOYDSENGG` has no feed row (probably a rights issue). Both are listed with that explanation in the report. Recent listings (`SSRETAIL`, `HEROMOTORS`, …) come out "too little shared history to compare" (under 20 bars), which is correct.

**Known limits:** closes only; dividends are not adjusted (a TradingView chart with dividend adjustment on shows small steady differences); the run is only as reliable as TradingView's own symbol resolution (a delisted/renamed ticker is reported as "not on TradingView"); the calendar rule needs ≥10 compared stocks; the report keeps at most 100 rows per table (the CSV has everything).
...
**Git-tracked data (`reference-data/`, `scanner-presets/`):** `CorporateActions.csv`, `DemergerAdjustments.csv` and `presets.json` were moved out of the git-ignored `NSE_DATA/` (and the project root) into folders that ARE tracked, because they can't be recovered later — NSE's corporate-actions feed is only a rolling window (the file is now a merged archive, see "Corporate-actions archive"), the corrections need TradingView (and can only be re-derived for the last ~370 days), and presets are user work. **Stable row order:** the downloader writes `CorporateActions.csv` sorted by ex-date, then symbol, then subject (then ISIN), because NSE returns rows that share an ex-date in an arbitrary order that changes between calls — without the sort every download looked "changed" (file rewritten, ~100 lines of git noise, and the bumped mtime made the server reprocess on every launch). With it, an unchanged feed leaves the file and its timestamp untouched (verified: 4 different input orders → byte-identical output). Bulk downloads (`NSE_DATA/`) stay git-ignored. Consequences: `git status` shows these files modified after a Refresh Data, a TradingView correction click or a preset save — commit them when it suits (`git add reference-data scanner-presets`). `src/nse_server.py` reads/writes the new paths (`REFERENCE_SUBDIR`, `PRESETS_FILE`), `needs_processing()` watches the two CSVs there, and `Download-NSE-Bhavcopy.ps1` writes `CorporateActions.csv` to `reference-data/`. **Startup migration:** `migrate_legacy_file()` (called from `run_server()` for each entry of `LEGACY_LOCATIONS`) is a one-time, idempotent, non-destructive move from the old paths — move if the new one is missing, delete an identical duplicate, and if both exist and differ keep the NEWER as live and set the older aside as a git-ignored `.bak` (`.replaced.bak` / `.superseded.bak`), so no data can be lost; it exists because a server started before the move (or the old downloader) can still be using the old paths. Not tracked (regenerable): `TradingViewVerification.json`, `processed_data.json`. Other files in `NSE_DATA/` worth a thought: `Sector-Stock-Mapping.csv` is a manual mapping (not from NSE) and `MidSmallcap400_Constituents.csv` has no point-in-time history — both are git-ignored today.

### Corporate-actions archive (`reference-data/CorporateActions.csv`)

NSE only serves a window of history, so an overwrite-on-download file silently loses events as they age out (and tracking it in git only preserves what was committed). `Download-NSE-Bhavcopy.ps1` therefore MERGES instead: it asks NSE for a window (default the last 3 years; `-CorpActionsFrom "01-Jan-2010"` for a one-time backfill of everything NSE still serves) and writes `archive = (stored rows OLDER than the window) ∪ (NSE's fresh rows)`, deduplicated and sorted (ex-date, symbol, subject, ISIN).
- **Inside the window NSE is authoritative** (replace, not append): a revised ex-date or a withdrawn event must take effect, otherwise the stale row would survive next to the corrected one and the price would be adjusted twice. **Older than the window nothing is ever deleted.** Rows whose ex-date can't be parsed are kept too.
- **Safety:** an empty feed, a failed download, or an implausibly small answer (fewer than half the stored in-window rows, when at least 20 are stored — a truncated response) changes nothing and warns. `Save-IfChanged` compares ignoring line endings, because git's `core.autocrlf` checks tracked files out with CRLF and that must not count as a change (it would rewrite the file, bump its mtime and force a reprocess).
- **Server guard:** `load_split_bonus_events(..., earliest_date_str)` (called with the first price date) ignores recognized split/bonus events older than every price we hold. Such an event can't change a stored price, but it WOULD switch on `bridge_split_induced_isin_changes()` for a stock with no in-window event, which stitches an older ISIN's history onto the current one. Verified on real data: a simulated fully backfilled archive (362 real + 310 synthetic old events, incl. old events for the bridging-sensitive stocks) leaves the new code's output identical, whereas without the guard `NEUEON`, `BURNPUR` and `MBECL` had their price history rewritten. Unhandled rows (demergers etc.) are not filtered by it. On today's real data the guard changes nothing (111 rows already predate the price data and affect no stock).
- **Growth:** ~120 rows a year; irrelevant for git and for processing.

### Server console freeze ("server is sticky") — QuickEdit

Symptom: the dashboard hangs and the server's console window looks stuck until someone clicks in it, after which a burst of log lines appears. Cause: Windows consoles have **QuickEdit** on by default — a click in the window starts a text selection, and while it is active every write to that console blocks. The server logs a line per API request *before* sending the response (`BaseHTTPRequestHandler.send_response` → `log_message`), so the request thread, and with it the dashboard, waits until the selection is cleared. Confirmed on the live server (its console mode was `0x1F7`, QuickEdit bit `0x40` set). Fixed in two ways: `disable_console_quickedit()` (called first in `run_server()`; `SetConsoleMode` on the console's input handle, Windows-only no-op otherwise or without a console) switches QuickEdit off for the server's window — verified on a real new console: `0x1F7` → `0x1B7` — and `NSEHandler.log_message` no longer logs `.../status` polls (the Data Quality tab and reprocess page poll every ~0.6 s for a whole run, thousands of lines). Needs a server restart to take effect. A console that is already frozen is released with Enter/Esc.
