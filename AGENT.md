...
│   ├── CorporateActions.csv      # Split/bonus/demerger corporate actions (ISIN, SYMBOL, EXDATE, SUBJECT). NSE serves only a rolling ~3-year window and the downloader overwrites the file each run, so git history is the long-term archive. Written by Download-NSE-Bhavcopy.ps1 — see "Stock Split/Bonus Price Adjustment"
